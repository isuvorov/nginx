...
3. `echo "include $HOME/projects/nginx/nginx.mac.conf;" > /usr/local/etc/nginx/nginx.conf`
4. `sudo cp /usr/local/opt/nginx/*.plist /Library/LaunchDaemons`
5. `sudo ln -s $HOME/projects/nginx /etc/nginx`
6. `sudo launchctl load -w /Library/LaunchDaemons/homebrew.mxcl.nginx.plist`
7. [How to start nginx on port 80 at Mac OSX boot up / log in](https://derickbailey.com/2014/12/27/how-to-start-nginx-on-port-80-at-mac-osx-boot-up-log-in/)
